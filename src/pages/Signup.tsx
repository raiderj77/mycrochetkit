...
export default function Signup() {
  ...
  useEffect(() => {
    ...
    if (ref && /^[A-Za-z0-9]{3,12}$/.test(ref)) {
      localStorage.setItem('mk_ref_code', ref.toUpperCase());
    }
  }, [searchParams]);

  // After user exists, write referredByCode to Firestore
  useEffect(() => {
    if (!user?.uid) return;

    const stored = (localStorage.getItem('mk_ref_code') || '').trim();
    if (!stored) return;
    ...
    setDoc(
      ...
      localStorage.removeItem('mk_ref_code');
    }).catch(() => { /* ignore */ });
  }, [user?.uid]);
