...
export function PatternTracker({
  ...
  const totalSteps = pattern.sections.reduce((sum, s) => sum + s.steps.length, 0);
  const completedSteps =
    pattern.sections
      .slice(0, progress.currentSectionIndex)
      .reduce((sum, s) => sum + s.steps.length, 0) + progress.currentStepIndex;
  const progressPercent = totalSteps > 0 ? (completedSteps / totalSteps) * 100 : 0;

  const canGoBack = progress.currentStepIndex > 0 || progress.currentSectionIndex > 0;
  const canGoForward =
    progress.currentSectionIndex < pattern.sections.length - 1 ||
    progress.currentStepIndex < currentSection?.steps.length - 1;
  ...
  }, [currentSection, progress, pattern.sections.length, onProgressChange]);
  ...
  }, [progress, pattern.sections, onProgressChange]);
  ...
  const renderInstruction = (instruction: string) => {
    ...
    return words.map((word, i) => {
      ...
      const definition = abbrevs[cleanWord] || abbrevs[cleanWord.replace('2', '')];
      if (definition) {
        return (
          <span
            key={i}
                      ...
                      {section.steps.map((step, stepIdx) => {
                        ...
                        return (
                          <button
                            ...
                            className={`w-full text-left px-3 py-2 rounded-lg text-sm transition-colors ${isCurrent ? 'bg-[#E86A58] text-white' : isPast ? 'bg-[#7FBFA0]/10 text-[#7FBFA0]' : 'bg-[#FFF8F0] text-[#2C1810]/70 hover:bg-[#F5E6E0]'}`}
                          >
                            <span className="font-medium">
                              {step.label || `Step ${stepIdx + 1}`}
                            </span>
                            {step.stitchCountEnd && (
                              <span className="ml-2 opacity-60">({step.stitchCountEnd})</span>
                            )}
                          </button>
                        );
                      })}
